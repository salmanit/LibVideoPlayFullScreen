...
public class NiceVideoChangeToast {
    Context context;
    ImageView iv_toast;
    ProgressBar pb_toast;

    public NiceVideoChangeToast(Context context) {
        this.context = context;
    }

    protected void volumeChange(int current, int max) {
        changeShow(R.drawable.vpf_card_player_gesture_sound_big, current, max);
    }

    protected void screenBrightnessChange(float current) {
        changeShow(R.drawable.vpf_card_player_gesture_bright_big, (int) (current * 100), 100);
    }
